...
class FalloutMinigame extends MiniGame {
    ...
    // Privates
    async processWords(length, count) {
        const l = length || 7;
        const c = count || 12;
    ...
    JTypeFill(containerID, text, TypeSpeed, callback, TypeCharacter, Prefix) {
    ...
    PrintWordsAndShit(container, words) {
    ...
    FillWordColumns() {
        ...
        i = 0;
        while (i < this.Words.length / 2) {
            var pos = start + i * Math.floor(AllChars.length / (this.Words.length / 2));
            for (var s = 0; s < this.Difficulty; s++) {
                var word = this.Words[i + this.Words.length / 2].toUpperCase();
                $(AllChars[pos + s]).addClass("word").text(word[s]).attr("data-word", word);
            }
            i++;
        }
    ...
    RemoveDud() {
        ...
        $("[data-word='" + WordToRemove + "']").each(() => {
            $(this).text(".").removeClass("word").removeAttr("data-word");
        });
    }

    SetupInteractions(column) {
        const _this = this;
        column = $(column);
