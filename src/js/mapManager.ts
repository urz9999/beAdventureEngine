...
export class MapManager {
  ...
  async loadMap(name: string): Promise<MapData> {
    const response = await fetch(`assets/maps/${name}.json`);
    this.map = await response.json();
    return this.map;
  }

  async loadLevel(number: number, spawnLocation?: [number, number]): Promise<void> {
    ...
    if (this.settings.partners.length > 0) {
      for (let i = 0; i < this.settings.partners.length; i++) {
        ...
        this.spriteManager.readPartnerCharacter(
          partner.id,
          partner.name,
          partner.walkingFrames,
          partner.idleFrames,
          partner.talkingFrames,
          partner.width,
          partner.height
        );
      }
    ...
    const questions = map.interactables.filter((int: Interactable) => int.type === 'question');
    for (let i = 0; i < questions.length; i++) {
      if (questions[i].question?.result) {
        const objects = questions[i].question!.result.filter((r: any) => r.object !== undefined);
    ...
    const combines = map.interactables.filter((int: Interactable) => int.type === 'combine');
    for (let i = 0; i < combines.length; i++) {
      if (combines[i].result) {
        const objects = combines[i].result!.filter((r: any) => r.object !== undefined);
        for (let j = 0; j < objects.length; j++) {
          this.spriteManager.readSprite(objects[j].object.name, `assets/images/ui/objects/${objects[j].object.name}.png`);
        }
      }
    ...
    // Load prize for look
    const looks = map.interactables.filter((int: Interactable) => int.type === 'look');
    for (let i = 0; i < looks.length; i++) {
      if (looks[i].result) {
        const objects = looks[i].result!.filter((r: any) => r.object !== undefined);
        for (let j = 0; j < objects.length; j++) {
          this.spriteManager.readSprite(objects[j].object.name, `assets/images/ui/objects/${objects[j].object.name}.png`);
        }
      }
    ...
    const partners = map.interactables.filter((int: Interactable) => int.type === 'partner');
    for (let i = 0; i < partners.length; i++) {
      if (partners[i].result) {
        const objects = partners[i].result!.filter((r: any) => r.object !== undefined);
        for (let j = 0; j < objects.length; j++) {
          this.spriteManager.readSprite(objects[j].object.name, `assets/images/ui/objects/${objects[j].object.name}.png`);
        }
      }
    ...
    // Load transition
    if (this.settings.worldTransition && this.settings.worldTransitionSWidth && this.settings.worldTransitionFrames) {
      this.spriteManager.readAnimatedCharacter(
        'transition',
        'assets/images/ui/menu/transition.png',
        this.settings.worldTransitionSWidth,
        this.settings.worldTransitionFrames
      );
    }
    ...
    this.gameVariables.player = {
      direction: true,
      animation: 'idle',
      noplayer: map.noplayer || false,
      initialOffsetX: 0,
      cam: 0
    };
    ...
    const waiter = setInterval(() => {
      if (this.spriteManager.allSpriteLoaded()) {
        ...
        if (bgM) {
          this.gameVariables.isMapBiggerThanCanvas = bgM.width > this.gameWidth;
        }

        // Set main position
        const startingLocation: [number, number] = spawnLocation ? [...spawnLocation] : [...map.startingPoint];
        ...
        const mainSprite = this.spriteManager.getSprite('main');
        if (mainSprite) {
          mainSprite.setCoords(startingLocation[0], startingLocation[1]);
        }
        ...
        for (let i = 0; i < this.settings.partners.length; i++) {
          ...
          if (partner) {
            partner.setCoords(
              startingLocation[0] + this.settings.partners[i].offsetX,
              startingLocation[1] + this.settings.partners[i].offsetY
            );
          }
        }
        ...
        for (let i = 0; i < this.gameVariables.characters.length; i++) {
          ...
          const charSprite = this.spriteManager.getSprite(char.name);
          if (charSprite) {
            charSprite.setCoords(char.x + initialOffsetX, char.y);
          }
        }
        ...
        for (let i = 0; i < this.gameVariables.objects.length; i++) {
          ...
          if (objSprite) {
            objSprite.setCoords(obj.x + initialOffsetX, obj.y);
          }
        }
  ...
  filterObjectsFromInventory(objects: GameObject[]): GameObject[] {
  ...
  filterInteractablesFromInventory(interactables: Interactable[]): Interactable[] {
