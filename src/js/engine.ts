...
export class beAdventurousEngine {
  ...
  registerMiniGame(name: string, miniGameClass: any): void {
    this.gameVariables.minigames[name] = new miniGameClass(
  ...
  private fixDpi(): void {
    let styleWidth: number, styleHeight: number;
    ...
    this.gameCanvas.setAttribute('width', (styleWidth * dpi).toString());
    this.gameCanvas.setAttribute('height', (styleHeight * dpi).toString());
  ...
  private drawLoader(): void {
    ...
    const loading = this.spriteManager.getSprite('Loading') as any;
  ...
  private drawMap(dt: number): void {
    ...
    if (!this.gameVariables.player.noplayer && this.gameVariables.isMapBiggerThanCanvas) {
      ...
      let cam = (currentPlayer as any)!.subSprite.dx + (currentPlayer as any)!.subSprite.width / 2 - this.gameWidth / 2;
    ...
    const bgR_Offset = !this.gameStatus.alternate ? (this.mapManager.map.bg_R_offset || 0) : (this.mapManager.map.bg_R_A_offset || 0);
    const bgM_Offset = !this.gameStatus.alternate ? (this.mapManager.map.bg_M_offset || 0) : (this.mapManager.map.bg_M_A_offset || 0);
    ...
    for (let i = 0; i < this.gameVariables.characters.length; i++) {
      ...
      const char = this.spriteManager.getSprite(name) as any;
    ...
    if (this.gameVariables.player.noplayer === false) {
      const player = this.spriteManager.getSprite('main') as any;
      ctx.drawImage(
        player.graphics,
      ...
      if (this.settings.partners.length > 0) {
        for (let i = 0; i < this.settings.partners.length; i++) {
          ...
          const partnerSprite = this.spriteManager.getSprite(partner.name) as any;
          ctx.drawImage(
    ...
    const bgF_Offset = !this.gameStatus.alternate ? (this.mapManager.map.bg_F_offset || 0) : (this.mapManager.map.bg_F_A_offset || 0);
    ...
    if (this.settings.worldTransition && !this.settings.worldTransitionDone) {
      const transition = this.spriteManager.getSprite('transition') as any;
    ...
    if (this.gameVariables.player.noplayer === false) {
      ...
      if (this.gameVariables.showMapName) {
        ...
        if (this.gameVariables.mapNameOpacity! > 0) {
          ...
          this.gameVariables.mapNameOpacity = this.mathHelper.lerp(this.gameVariables.mapNameOpacity || 0, 0, 0.3);
  ...
  private animateLoader(): void {
    ...
    if (loading && 'animate' in loading) {
      (loading as any).animate();
    }
  }
  ...
  private animateMap(): void {
    ...
    switch (animation) {
      case 'idle': {
        if (!direction) {
          player.playLeftIdle();
          for (let i = 0; i < this.settings.partners.length; i++) {
            const partner = this.spriteManager.getSprite(this.settings.partners[i].name) as any;
      ...
      case 'talking': {
        if (!direction) {
          ...
          for (let i = 0; i < this.settings.partners.length; i++) {
            const partner = this.spriteManager.getSprite(this.settings.partners[i].name) as any;
            partner.playleftTalking();
          }
        } else {
          player.playRightTalking();
          for (let i = 0; i < this.settings.partners.length; i++) {
            const partner = this.spriteManager.getSprite(this.settings.partners[i].name) as any;
            partner.playRightTalking();
          }
        }
        break;
      }
    ...
    if (animation === 'walking' || this.gameStatus.correctingView) {
      // process movement
      const x = this.gameStatus.partnerIndex === 0 ? player.getCoords().x : (this.spriteManager.getSprite(this.settings.partners[this.gameStatus.partnerIndex - 1].name) as any).getCoords().x;
